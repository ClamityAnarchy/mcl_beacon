...
local function get_base_formspec()
	if base_formspec == "" then
		base_formspec =
			...
			"label[4,0;Upgrades]"..
			"list[context;mcl_beacon_upgrades;4,0.5;4,1;]"..
			"listring[context;mcl_beacon_upgrades]"..
			"list[current_player;main;0,4.25;8,4;]"..
			"listring[current_player;main]"..
			"item_image[4,0.5;1,1;"..mcl_beacon.config.upgrade_item.."]"..
			"item_image[5,0.5;1,1;"..mcl_beacon.config.upgrade_item.."]"..
			"item_image[6,0.5;1,1;"..mcl_beacon.config.upgrade_item.."]"..
			"item_image[7,0.5;1,1;"..mcl_beacon.config.upgrade_item.."]"..
			"button_exit[4,2.875;4,1;save;Save]"
		if mcl_beacon.has_digilines then
...
function mcl_beacon.update_formspec(pos)
	local meta = minetest.get_meta(pos)
	local level = mcl_beacon.get_level(pos)
	local effect_names, effect_ids = mcl_beacon.get_effects_for_level(level)
	local max_range = mcl_beacon.config["effect_range_"..level]
	local effect = meta:get_string("effect")

	meta:set_int("range", mcl_beacon.limit_range(meta:get_int("range"), level))
...
function mcl_beacon.receive_fields(pos, formname, fields, player)
	...
	if fields.range then
		meta:set_int("range", mcl_beacon.limit_range(fields.range, level))
	end
